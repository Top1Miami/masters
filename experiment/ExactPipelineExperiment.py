...
class ExactPipelineExperiment(Experiment):
    """
    ...
    def run(self, features, labels, file_name):
        ...
        for _ in range(self.generated_samples_number):
            ...
            train_scoring = ExactPipelineExperiment.exact_rec_wrapper(train_known_features)
            test_scoring = ExactPipelineExperiment.exact_rec_wrapper(test_known_features)
            test_precision_scoring = ExactPipelineExperiment.exact_prec_wrapper(test_known_features)
        ...
        plot_no_std_no_color(df, self.save_path.format(subname) + 'exact_feature_recall_std.png', 'recall_score',
                             self.max_features_select)
        plot_no_std_no_color(df, self.save_path.format(subname) + 'exact_estimator_score_std.png', 'estimator_score',
                             self.max_features_select)
        plot_no_std_no_color(df, self.save_path.format(subname) + 'exact_feature_precision_std.png', 'precision_score',
                             self.max_features_select)
        df.to_csv(self.save_path.format(subname) + 'exact_comparison.csv')
